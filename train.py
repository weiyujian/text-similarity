...
tf.flags.DEFINE_string("train_file", "./data/cnews.train.seg", "train data for Chinese.")#'/home/jwy/insuranceQA-cnn-lstm-master/insuranceQA/train'
tf.flags.DEFINE_string("dev_file", "./data/cnews.test.seg", "val data for Chinese.")
tf.flags.DEFINE_string("test_file", "./data/cnews.test.seg", "test data for Chinese.")
...
tf.flags.DEFINE_integer("max_seq_len", 1000, "Max document length(default: 600)")
tf.flags.DEFINE_string("model_version", "", "model version")
tf.flags.DEFINE_string("embedding_type", "char_embedding", "embedding type, word_embedding, char_embedding, subword_embedding")
...
tf.flags.DEFINE_string("sample_type", "rand", "[semi] for semi-hard negative sample, [rand] for random sample (default: 'semi')")
...
if FLAGS.embedding_type == "char_embedding":
	...
	vocab = insurance_qa_data_helpers.build_vocab(FLAGS.train_file)
else:
  ...
  with tf.device("/gpu:1"):
	...
	with sess.as_default():
		...
		def dev_step(testList):
		  ...
		  i = int(0)
		  while True:
			  x_test_1, x_test_2, tag_list = insurance_qa_data_helpers.load_data_val(testList, vocab, i, FLAGS.batch_size, max_seq_len, FLAGS.embedding_type)
			  feed_dict = {
		...
		def max_eval(testraw_list, score_list):
			...
			for inx in xrange(len(testraw_list)):
				...
				if query not in max_cand or score_list[inx] > max_cand[query][0]:
					max_cand[query] = (score_list[inx],label)
			acc_cnt = 0
			pred_list = []
		...
		for i in range(FLAGS.num_epochs):
			try:
				if FLAGS.sample_type == 'rand':
					x_batch_1, x_batch_2, x_batch_3 = insurance_qa_data_helpers.rand_sample(vocab, alist, raw, FLAGS.batch_size, max_seq_len, FLAGS.embedding_type)
				else:
					x_batch_1, x_batch_2, x_batch_3 = insurance_qa_data_helpers.semihard_sample(sess, cnn, vocab, raw, FLAGS.batch_size, max_seq_len, avec_list, label_map, alist, FLAGS.macro_batch_size, FLAGS.min_margin, FLAGS.max_margin, FLAGS.embedding_type)
